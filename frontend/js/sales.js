...
function filteredSales() { const from = saleDateFromFilter?.value || ""; const to = saleDateToFilter?.value || ""; return sales.filter((sale) => { const key = dateKey(sale.document_date); if (from && key && key < from) return false; if (to && key && key > to) return false; return true; }); }
...
if (resetSalesFiltersBtn) resetSalesFiltersBtn.addEventListener("click", async () => { saleStatusFilter.value = ""; saleTypeFilter.value = ""; if (saleDateFromFilter) saleDateFromFilter.value = ""; if (saleDateToFilter) saleDateToFilter.value = ""; await loadSales(); });
