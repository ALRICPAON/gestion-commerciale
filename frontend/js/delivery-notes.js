...
async function loadLabels() { if (!selectedDeliveryNote) return; try { const response = await apiFetch(`${API_BASE_URL}/api/delivery-notes/${selectedDeliveryNote.id}/health-labels`); if (!response) return; const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data.error || 'Erreur préparation étiquettes'); const labels = data.labels || []; detailContent.innerHTML = window.HealthLabels.renderPreview(labels, data.zpl_document, data.warnings || []); window.HealthLabels.bindZplDownload(detailContent, data.zpl_document, `${selectedDeliveryNote.reference_number || selectedDeliveryNote.id}-etiquettes.zpl`); detailTitle.textContent = 'Étiquettes sanitaires'; detailSubtitle.textContent = `${labels.length} étiquette(s) 100 x 100 mm préparée(s)`; } catch (err) { console.error('Erreur étiquettes sanitaires :', err); showFeedback(err.message || 'Erreur étiquettes', 'error'); } }
