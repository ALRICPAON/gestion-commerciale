...
function healthLabelsModule(requiredMethod) { const module = window.HealthLabels; if (module && (!requiredMethod || typeof module[requiredMethod] === 'function')) return module; const error = new Error(`Module HealthLabels indisponible${requiredMethod ? `: ${requiredMethod}` : ''}`); console.error('Erreur module etiquettes sanitaires :', error); showFeedback('Module d impression des etiquettes sanitaires non charge. Rechargez la page puis reessayez.', 'error'); return null; }
async function loadLabels() { if (!selectedDeliveryNote) return; try { const labelsModule = healthLabelsModule('renderPreview'); if (!labelsModule) return; const response = await apiFetch(`${API_BASE_URL}/api/delivery-notes/${selectedDeliveryNote.id}/health-labels`); if (!response) return; const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data.error || 'Erreur préparation étiquettes'); const labels = data.labels || []; const printer = labels[0]?.printer || {}; const visualWidth = printer.visual_width_mm || printer.height_mm || 150; const visualHeight = printer.visual_height_mm || printer.width_mm || 70; detailContent.innerHTML = labelsModule.renderPreview(labels, data.zpl_document, data.warnings || []); if (typeof labelsModule.bindZplDownload === 'function') labelsModule.bindZplDownload(detailContent, data.zpl_document, `${selectedDeliveryNote.reference_number || selectedDeliveryNote.id}-etiquettes.zpl`); detailTitle.textContent = 'Étiquettes sanitaires'; detailSubtitle.textContent = `${labels.length} étiquette(s) ${visualWidth} x ${visualHeight} mm préparée(s)`; } catch (err) { console.error('Erreur étiquettes sanitaires :', err); showFeedback(err.message || 'Erreur étiquettes', 'error'); } }
