...
function buildAltaSupplierInvoicePayload(pennylaneInvoice, matchResults) {
  const documentType = normalizeSupplierDocumentType(pennylaneInvoice.document_type) || 'invoice';
  return {
    source: 'pennylane_supplier_invoice',
    document_type: documentType,
    credit_note_reason: documentType === 'credit_note' ? normalizeCreditNoteReason(pennylaneInvoice.credit_note_reason) : null,
    ...
    amount_ex_vat: documentType === 'credit_note' ? positiveAmount(pennylaneInvoice.amount_ex_vat ?? pennylaneInvoice.currency_amount_ex_vat) : num(pennylaneInvoice.amount_ex_vat ?? pennylaneInvoice.currency_amount_ex_vat),
    amount_vat: documentType === 'credit_note' ? positiveAmount(pennylaneInvoice.amount_vat ?? pennylaneInvoice.currency_amount_vat) : num(pennylaneInvoice.amount_vat ?? pennylaneInvoice.currency_amount_vat),
    amount_inc_vat: documentType === 'credit_note' ? positiveAmount(pennylaneInvoice.amount_inc_vat ?? pennylaneInvoice.currency_amount_inc_vat) : num(pennylaneInvoice.amount_inc_vat ?? pennylaneInvoice.currency_amount_inc_vat),
    financial_effect_ex_vat: signedFinancialAmount(pennylaneInvoice, 'amount_ex_vat'),
...
async function findOrCreateAltaSupplierInvoice(client, { pennylaneInvoice, matchResults, user }) {
  ...
  const documentType = normalizeSupplierDocumentType(pennylaneInvoice.document_type) || 'invoice';
  const creditNoteReason = documentType === 'credit_note' ? normalizeCreditNoteReason(pennylaneInvoice.credit_note_reason) : null;
  const totalExVat = documentType === 'credit_note' ? positiveAmount(pennylaneInvoice.amount_ex_vat ?? pennylaneInvoice.currency_amount_ex_vat) : num(pennylaneInvoice.amount_ex_vat ?? pennylaneInvoice.currency_amount_ex_vat);
  const vatAmount = documentType === 'credit_note' ? positiveAmount(pennylaneInvoice.amount_vat ?? pennylaneInvoice.currency_amount_vat) : num(pennylaneInvoice.amount_vat ?? pennylaneInvoice.currency_amount_vat);
  const totalIncVat = documentType === 'credit_note' ? positiveAmount(pennylaneInvoice.amount_inc_vat ?? pennylaneInvoice.currency_amount_inc_vat) : num(pennylaneInvoice.amount_inc_vat ?? pennylaneInvoice.currency_amount_inc_vat);
  const proposedStatus = documentType === 'credit_note' ? 'draft' : (Number(pennylaneInvoice.auto_anomaly_count || 0) > 0 ? 'invoice_difference' : 'matched');
  const proposedMatchStatus = documentType === 'credit_note' ? 'partial' : (Number(pennylaneInvoice.auto_anomaly_count || 0) > 0 ? 'discrepancy' : 'matched');
...
async function replaceAltaProposedMatches(client, { altaInvoice, pennylaneInvoice, matchResults }) {
  ...
  if (isSupplierCreditNote(altaInvoice)) return 0;
  ...
  try {
    ...
    const result = await req.dbPool.query(
      ...
      SELECT
        ...
        psi.amount_inc_vat,
        psi.document_type,
        psi.credit_note_reason,
        psi.source_supplier_invoice_id,
        psi.source_purchase_id,
        psi.stock_effect,
