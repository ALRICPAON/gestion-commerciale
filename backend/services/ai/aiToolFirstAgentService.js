...
function ranked(rows, query, valuesForRow, limit) {
  return rows
    .map((row) => ({ ...row, match_score: score(query, valuesForRow(row)) }))
    .filter((row) => !query || row.match_score > 0)
    .sort((a, b) => b.match_score - a.match_score || String(a.name || a.designation || '').localeCompare(String(b.name || b.designation || '')))
    .slice(0, limit);
}
...
function quantity(value, unit = 'kg') {
...
function pendingSummary(payload) {
  ...
  return [
    ...
    ...lines.map((line) => {
      ...
      const stock = line.stock_quantity !== undefined ? ` - stock ${quantity(line.stock_quantity, line.sale_unit || 'kg')}` : '';
      const source = line.source ? ` - source ${line.source}` : '';
      const packageText = line.package_count > 0 && line.weight_per_package > 0
        ? `${line.package_count} colis x ${line.weight_per_package} kg = ${quantity(line.quantity, line.sale_unit || 'kg')}`
        : quantity(line.quantity, line.sale_unit || 'kg');
      return `- ${line.article_label}${plu} : ${packageText} a ${money(line.unit_sale_price_ht)}/${line.sale_unit || 'kg'}${stock}${source}`;
    }),
    '',
    'Confirmer l action ?',
  ].join('\n');
  ...
  return [
    {
      ...
      function: {
        ...
        description: 'Recherche large et normalisee par domaine metier, sans SQL libre.',
        parameters: {
          type: 'object',
          properties: {
            domain: { type: 'string', enum: ['clients', 'articles', 'stock', 'suppliers', 'sales', 'purchases'] },
            query: { type: 'string' },
            filters: {
              type: 'object',
              properties: {
    ...
    { type: 'function', function: { name: 'get_client_profile', description: 'Fiche client normalisee.', parameters: idArg('client_id') } },
    { type: 'function', function: { name: 'get_article_profile', description: 'Fiche article normalisee avec stock et historiques.', parameters: idArg('article_id') } },
    { type: 'function', function: { name: 'get_stock_state', description: 'Etat de stock detaille d un article.', parameters: idArg('article_id') } },
    { type: 'function', function: { name: 'get_supplier_profile', description: 'Fiche fournisseur normalisee.', parameters: idArg('supplier_id') } },
    {
      type: 'function',
      function: {
        name: 'get_sales_history',
        description: 'Historique ventes filtre par client et/ou article.',
        parameters: { type: 'object', properties: { client_id: { type: 'string' }, article_id: { type: 'string' }, query: { type: 'string' }, limit: { type: 'integer', minimum: 1, maximum: 80 } } },
      },
    },
    {
      type: 'function',
      function: {
        ...
        description: 'Historique achat/reception connu par article et/ou fournisseur.',
        parameters: { type: 'object', properties: { article_id: { type: 'string' }, supplier_id: { type: 'string' }, query: { type: 'string' }, limit: { type: 'integer', minimum: 1, maximum: 80 } } },
      },
    },
    { type: 'function', function: { name: 'search_clients', description: 'Alias de recherche clients.', parameters: { type: 'object', properties: { query: { type: 'string' }, limit: { type: 'integer', minimum: 1, maximum: 20 } }, required: ['query'] } } },
    { type: 'function', function: { name: 'search_articles', description: 'Alias de recherche articles.', parameters: { type: 'object', properties: { query: { type: 'string' }, limit: { type: 'integer', minimum: 1, maximum: 20 } }, required: ['query'] } } },
    { type: 'function', function: { name: 'search_stock', description: 'Alias de recherche stock.', parameters: { type: 'object', properties: { query: { type: 'string' }, article_ids: { type: 'array', items: { type: 'string' } }, limit: { type: 'integer', minimum: 1, maximum: 30 } } } } },
    { type: 'function', function: { name: 'search_suppliers', description: 'Alias de recherche fournisseurs.', parameters: { type: 'object', properties: { query: { type: 'string' }, limit: { type: 'integer', minimum: 1, maximum: 20 } }, required: ['query'] } } },
    { type: 'function', function: { name: 'get_user_memory', description: 'Memoire utilisateur en contexte seulement.', parameters: { type: 'object', properties: {} } } },
    { type: 'function', function: { name: 'get_client_memory', description: 'Memoire client en contexte seulement.', parameters: idArg('client_id') } },
    {
      type: 'function',
      function: {
        name: 'prepare_customer_order_draft',
        description: 'Prepare une commande brouillon avec client_id et article_id exacts. Ne cree pas la commande.',
        parameters: {
          type: 'object',
          properties: {
            client_id: { type: 'string' },
    ...
    { type: 'function', function: { name: 'prepare_supplier_order', description: 'Contrat futur, non executable dans ce socle.', parameters: { type: 'object', properties: {} } } },
    { type: 'function', function: { name: 'prepare_email_draft', description: 'Contrat futur, non executable dans ce socle.', parameters: { type: 'object', properties: {} } } },
    { type: 'function', function: { name: 'prepare_commercial_offer', description: 'Contrat futur, non executable dans ce socle.', parameters: { type: 'object', properties: {} } } },
    { type: 'function', function: { name: 'create_pending_action', description: 'Fige une action preparee et demande confirmation humaine.', parameters: { type: 'object', properties: { action_type: { type: 'string', enum: ['customer_order_draft'] }, payload: { type: 'object' } }, required: ['action_type', 'payload'] } } },
...
async function searchBusinessData({ db, user, args }) {
  ...
  if (domain === 'clients') {
    const res = await db.query(`
      ...
      FROM clients
      WHERE store_id = $1 AND ($2::text IS NULL OR COALESCE(status, 'active') = $2)
      ORDER BY name ASC LIMIT 300
    `, [user.store_id, filters.status || null]);
    const rows = ranked(res.rows, query, (row) => [row.name, row.code, row.city, row.email], limit);
    return ok({ domain, query, results: rows.map((row) => businessResult(domain, row)) });
  }

  if (domain === 'articles' || domain === 'stock') {
    const onlyStock = domain === 'stock' || Boolean(filters.only_in_stock);
    const res = await db.query(`
      SELECT a.id, a.plu, a.ean, a.designation, a.display_name, a.unit, a.sale_unit, a.vat_rate,
             a.sale_price_ex_vat, a.sale_price_level_1_ht, a.sale_price_level_2_ht, a.sale_price_level_3_ht,
             COALESCE(a.is_active, true) AS is_active, COALESCE(ss.stock_quantity, 0) AS stock_quantity,
             COALESCE(ss.pma, 0) AS pma, ss.next_dlc
      FROM articles a
      LEFT JOIN stock_summary ss ON ss.article_id = a.id AND ss.store_id = a.store_id
      WHERE a.store_id = $1 AND COALESCE(a.is_active, true) = true
        AND ($2::boolean = false OR COALESCE(ss.stock_quantity, 0) > 0)
      ORDER BY a.designation ASC LIMIT 500
    `, [user.store_id, onlyStock]);
    const rows = ranked(res.rows, query, (row) => [row.designation, row.display_name, row.plu, row.ean], limit);
    return ok({ domain, query, results: rows.map((row) => businessResult(domain, row)) });
  }
  ...
  if (domain === 'sales') return getSalesHistory({ db, user, args: { client_id: filters.client_id, article_id: filters.article_id, query, limit } });
  if (domain === 'purchases') return getPurchaseHistory({ db, user, args: { article_id: filters.article_id, supplier_id: filters.supplier_id, query, limit } });
  return ok({ domain, query, results: [], reason: 'Domaine non pris en charge.' });
}

function aliasSearch(domain, args) {
  return { domain, query: args.query || '', filters: { limit: args.limit, only_in_stock: domain === 'stock' } };
}
...
async function getClientProfile({ db, user, args }) {
  ...
  const client = res.rows[0] || null;
  if (!client) return ok({ client: null });
  const sales = await getSalesHistory({ db, user, args: { client_id: args.client_id, limit: 20 } });
  return ok({ client: businessResult('clients', client), sales_history: sales.sales_history });
}
...
async function getArticleProfile({ db, user, args }) {
  const res = await db.query(`
    SELECT a.id, a.plu, a.ean, a.designation, a.display_name, a.unit, a.sale_unit, a.vat_rate,
           a.sale_price_ex_vat, a.sale_price_level_1_ht, a.sale_price_level_2_ht, a.sale_price_level_3_ht,
           COALESCE(a.is_active, true) AS is_active, COALESCE(ss.stock_quantity, 0) AS stock_quantity,
           COALESCE(ss.stock_value_ex_vat, 0) AS stock_value_ex_vat, COALESCE(ss.pma, 0) AS pma, ss.next_dlc
    FROM articles a
    LEFT JOIN stock_summary ss ON ss.article_id = a.id AND ss.store_id = a.store_id
    WHERE a.id = $1 AND a.store_id = $2 LIMIT 1
  ...
  const [stock, sales, purchases] = await Promise.all([
    ...
    getPurchaseHistory({ db, user, args: { article_id: args.article_id, limit: 20 } }),
  ]);
  return ok({ article: businessResult('articles', article), stock: stock.stock_state, sales_history: sales.sales_history, purchase_history: purchases.purchase_history });
}
  ...
  const [summary, lots] = await Promise.all([
    db.query(`
      SELECT a.id AS article_id, a.plu, a.designation, a.unit, a.sale_unit,
             COALESCE(ss.stock_quantity, 0) AS stock_quantity, COALESCE(ss.stock_value_ex_vat, 0) AS stock_value_ex_vat,
             COALESCE(ss.pma, 0) AS pma, ss.next_dlc
      FROM articles a
      LEFT JOIN stock_summary ss ON ss.article_id = a.id AND ss.store_id = a.store_id
      WHERE a.id = $1 AND a.store_id = $2 LIMIT 1
    `, [args.article_id, user.store_id]),
    db.query(`
      SELECT l.id AS lot_id, l.lot_code, l.supplier_lot_number, l.qty_remaining, l.unit_cost_ex_vat, l.dlc,
      ...
      ORDER BY COALESCE(l.dlc, DATE '9999-12-31') ASC, l.qty_remaining DESC LIMIT 30
    `, [user.store_id, args.article_id]),
  ]);
  const row = summary.rows[0] || null;
  return ok({ stock_state: row ? { article_id: row.article_id, plu: row.plu, designation: row.designation, quantity: number(row.stock_quantity), unit: row.sale_unit || row.unit || 'kg', value_ht: number(row.stock_value_ex_vat), pma: number(row.pma), next_dlc: row.next_dlc || null, lots: lots.rows } : null });
}

async function getSupplierProfile({ db, user, args }) {
  const res = await db.query(`
    SELECT id, code, name, COALESCE(status, 'active') AS status, city, email, phone
    FROM suppliers
    WHERE id = $1 AND store_id = $2 LIMIT 1
  `, [args.supplier_id, user.store_id]);
  const supplier = res.rows[0] || null;
  if (!supplier) return ok({ supplier: null });
  const purchases = await getPurchaseHistory({ db, user, args: { supplier_id: args.supplier_id, limit: 20 } });
  return ok({ supplier: businessResult('suppliers', supplier), purchase_history: purchases.purchase_history });
}
...
async function getSalesHistory({ db, user, args }) {
  ...
  const res = await db.query(`
    ...
    WHERE ${filters.join(' AND ')} AND COALESCE(sd.status, '') NOT IN ('cancelled')
    ORDER BY sd.document_date DESC, sl.article_label ASC LIMIT $${params.length}
  `, params);
  const rows = args.query ? ranked(res.rows, args.query, (row) => [row.client_name, row.article_label, row.article_plu], limit) : res.rows;
  return ok({ sales_history: rows.map((row) => ({ document_id: row.document_id, document_date: row.document_date, document_type: row.document_type, status: row.status, client: { id: row.client_id, code: row.client_code, name: row.client_name }, article: { id: row.article_id, plu: row.article_plu, label: row.article_label }, quantity: number(row.sold_quantity), unit: row.sale_unit, unit_price_ht: number(row.unit_sale_price_ht), amount_ht: number(row.line_amount_ht), margin_ht: number(row.line_margin_ex_vat) })) });
}

async function getPurchaseHistory({ db, user, args }) {
  const limit = Math.min(number(args.limit, 30), 80);
  const params = [user.store_id];
  const filters = ['l.store_id = $1'];
  if (args.article_id) { params.push(args.article_id); filters.push(`l.article_id = $${params.length}`); }
  if (args.supplier_id) { params.push(args.supplier_id); filters.push(`l.supplier_id = $${params.length}`); }
  params.push(limit);
  const res = await db.query(`
    SELECT l.id AS lot_id, l.lot_code, l.supplier_lot_number, l.qty_remaining, l.unit_cost_ex_vat, l.dlc,
           a.id AS article_id, a.plu AS article_plu, a.designation AS article_label, a.sale_unit,
    ...
    ORDER BY COALESCE(l.dlc, DATE '9999-12-31') DESC, a.designation ASC LIMIT $${params.length}
  `, params);
  const rows = args.query ? ranked(res.rows, args.query, (row) => [row.article_label, row.article_plu, row.supplier_name, row.lot_code], limit) : res.rows;
  return ok({ purchase_history: rows.map((row) => ({ lot_id: row.lot_id, lot_code: row.lot_code, supplier_lot_number: row.supplier_lot_number, dlc: row.dlc, article: { id: row.article_id, plu: row.article_plu, label: row.article_label, unit: row.sale_unit || 'kg' }, supplier: { id: row.supplier_id, code: row.supplier_code, name: row.supplier_name }, quantity_remaining: number(row.qty_remaining), unit_cost_ex_vat: number(row.unit_cost_ex_vat) })) });
}
...
async function getUserMemory({ db, user }) {
  ...
  return ok({ memory: res.rows, context_only: true });
}
...
async function prepareCustomerOrderDraft({ db, user, args }) {
  const clientRes = await db.query(`
    ...
    FROM clients
    WHERE id = $1 AND store_id = $2 AND COALESCE(status, 'active') <> 'inactive' LIMIT 1
  `, [args.client_id, user.store_id]);
  ...
  for (const rawLine of rawLines) {
    ...
    const articleRes = await db.query(`
      ...
      FROM articles a
      LEFT JOIN stock_summary ss ON ss.article_id = a.id AND ss.store_id = a.store_id
      WHERE a.id = $1 AND a.store_id = $2 AND COALESCE(a.is_active, true) = true LIMIT 1
    ...
    const isNegoce = Boolean(rawLine.allow_negoce) || stockQty < qty;
    const line = {
  ...
  const payload = { client, lines, has_negoce_lines: lines.some((line) => line.is_negoce), prepared_by: 'ai_tool_first_agent', prepared_at: new Date().toISOString() };
...
async function createPendingAction({ db, user, args }) {
  ...
  console.info('[AI ACTION] pending confirmation created', { store_id: user.store_id, user_id: user.id, action_id: res.rows[0].id, action_type: args.action_type });
  return ok({ pending_action: { id: res.rows[0].id, action_type: res.rows[0].action_type, status: res.rows[0].status, summary, payload, created_at: res.rows[0].created_at } });
}
...
async function unavailablePreparation({ name }) {
...
const HANDLERS = {
  ...
  get_supplier_profile: getSupplierProfile,
  get_sales_history: getSalesHistory,
  get_purchase_history: getPurchaseHistory,
  search_clients: ({ db, user, args }) => searchBusinessData({ db, user, args: aliasSearch('clients', args) }),
  search_articles: ({ db, user, args }) => searchBusinessData({ db, user, args: aliasSearch('articles', args) }),
  search_stock: ({ db, user, args }) => args.article_ids?.length ? getStockState({ db, user, args: { article_id: args.article_ids[0] } }) : searchBusinessData({ db, user, args: aliasSearch('stock', args) }),
  search_suppliers: ({ db, user, args }) => searchBusinessData({ db, user, args: aliasSearch('suppliers', args) }),
  get_user_memory: getUserMemory,
  get_client_memory: getClientMemory,
  prepare_customer_order_draft: prepareCustomerOrderDraft,
  prepare_supplier_order: unavailablePreparation,
  prepare_email_draft: unavailablePreparation,
  prepare_commercial_offer: unavailablePreparation,
...
async function executeTool({ db, user, toolCall }) {
  ...
  try {
    ...
    console.error('[AI TOOL FIRST] tool error', { store_id: user.store_id, user_id: user.id, tool: name, message: error.message, code: error.code || null });
...
function isConfirmationIntent(prompt) {
  ...
  return ['oui', 'ok', 'confirme', 'je confirme', 'vas y', 'valide', 'tu peux valider'].includes(value);
}
