...
const tools = [
  ...
  tool({
    ...
    description: 'Prepare l ajout d un article ALTA dans une session brouillon avec fournisseur, prix achat, transport et tarifs dynamiques.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.LOW_REVERSIBLE_WRITE,
    requiredPermission: 'pricing.write',
    requiresConfirmation: false,
    inputSchema: pricingLineMutationSchema,
    execute: async ({ context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Ajout ligne tarification prepare', data: { prepared_action: pricingPreparedAction(context, input, 'pricing.line.add', 'Ajouter une ligne tarification', 'La ligne sera ajoutee au brouillon apres confirmation.', [{ type: 'pricing_session', id: input.pricing_session_id }]), confirmation_tool: 'create_pending_action', action_type: 'pricing.line.add', payload: input } }),
  }),
  tool({
    name: 'prepare_pricing_line_update',
    title: 'Preparer modification ligne tarification',
    description: 'Prepare une modification de ligne: prix achat, transport, fournisseur retenu, exclusion mercuriale et tarifs dynamiques. Les champs absents ne changent pas.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.LOW_REVERSIBLE_WRITE,
    requiredPermission: 'pricing.write',
    requiresConfirmation: false,
    inputSchema: { ...pricingLineMutationSchema, required: ['pricing_line_id'] },
    execute: async ({ context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Modification ligne tarification preparee', data: { prepared_action: pricingPreparedAction(context, input, 'pricing.line.update', 'Modifier une ligne tarification', 'La ligne brouillon sera modifiee apres confirmation.', [{ type: 'pricing_line', id: input.pricing_line_id }]), confirmation_tool: 'create_pending_action', action_type: 'pricing.line.update', payload: input } }),
  }),
  tool({
