...
const pricingReadInputSchema = {
  type: 'object',
  properties: {
    id: { type: 'string' },
    pricing_session_id: { type: 'string' },
    pricing_line_id: { type: 'string' },
    date: { type: 'string', maxLength: 10 },
    pricing_date: { type: 'string', maxLength: 10 },
    status: { type: 'string' },
    query: { type: 'string', maxLength: 200 },
    supplier_id: { type: 'string' },
    import_id: { type: 'string' },
    supplier_price_import_id: { type: 'string' },
    user_decision: { type: 'string', enum: ['pending', 'confirmed', 'overridden', 'ignored'] },
    match_status: { type: 'string', enum: ['certain', 'probable', 'unrecognized', 'ignored'] },
    unresolved: { type: 'boolean' },
    article_id: { type: 'string' },
    missing_legacy_level: { type: 'integer', minimum: 1 },
    limit: { type: 'integer', minimum: 1, maximum: 1000 },
  },
  additionalProperties: false,
};

const pricingTariffInputSchema = {
  type: 'object',
  properties: {
    tariff_level_id: { type: 'string' },
    legacy_level: { type: 'integer', minimum: 1 },
    code: { type: 'string' },
    price_ht: { type: ['number', 'string', 'null'] },
    source: { type: 'string' },
  },
  additionalProperties: false,
};

const pricingLineMutationSchema = {
  type: 'object',
  properties: {
    pricing_session_id: { type: 'string' },
    pricing_line_id: { type: 'string' },
    ...
    supplier_id: { type: ['string', 'null'] },
    purchase_price_ht: { type: ['number', 'string', 'null'] },
    transport_cost_ht: { type: ['number', 'string', 'null'] },
    create_revision_if_published: { type: 'boolean' },
    tariffs: { type: 'array', items: pricingTariffInputSchema },
    exclude_from_mercuriale: { type: 'boolean' },
    notes: { type: ['string', 'null'] },
  },
  additionalProperties: false,
};

function pricingPreparedAction(context, input, actionType, summary, impact, targetObjects = []) {
  return preparedBusinessAction(context, {
    payload: input,
    summary,
    impact,
    ...
    required_permissions: ['mcp.execute', 'pricing.write'],
    summary,
    impact,
  });
}
...
const tools = [
  ...
  tool({
    name: 'list_pricing_sessions',
    title: 'Lister tarifications',
    description: 'Liste les sessions Tarification / Cours du jour du magasin courant. Source primaire des prix commerciaux du jour.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.READ,
    requiredPermission: 'pricing.read',
    requiresConfirmation: false,
    inputSchema: pricingReadInputSchema,
    execute: async ({ db, context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Sessions tarification', data: await pricing.listPricingSessions(db, context.store_id, input) }),
  }),
  tool({
    name: 'get_pricing_session',
    title: 'Lire tarification',
    description: 'Ouvre une session de tarification et retourne ses lignes avec cout rendu et tarifs dynamiques.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.READ,
    requiredPermission: 'pricing.read',
    requiresConfirmation: false,
    inputSchema: pricingReadInputSchema,
    execute: async ({ db, context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Detail tarification', data: await pricing.getPricingSession(db, context.store_id, input) }),
  }),
  tool({
    name: 'get_current_pricing_session',
    title: 'Tarification publiee courante',
    description: 'Lit la publication active par magasin/date. Une seule session publiee active peut exister pour une date.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.READ,
    requiredPermission: 'pricing.read',
    requiresConfirmation: false,
    inputSchema: pricingReadInputSchema,
    execute: async ({ db, context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Tarification publiee active', data: await pricing.getCurrentPricingSession(db, context.store_id, input) }),
  }),
  tool({
    name: 'list_pricing_lines',
    title: 'Lister lignes tarification',
    description: 'Liste les lignes de tarification avec filtres session, article, fournisseur, recherche texte et lignes sans prix.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.READ,
    requiredPermission: 'pricing.read',
    requiresConfirmation: false,
    inputSchema: pricingReadInputSchema,
    execute: async ({ db, context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Lignes tarification', data: await pricing.listPricingLines(db, context.store_id, input) }),
  }),
  tool({
    name: 'search_pricing_lines',
    title: 'Rechercher tarification',
    description: 'Recherche naturelle dans les lignes de tarification par designation, PLU, fournisseur, article ou statut de prix.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.READ,
    requiredPermission: 'pricing.read',
    requiresConfirmation: false,
    inputSchema: pricingReadInputSchema,
    execute: async ({ db, context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Recherche tarification', data: await pricing.listPricingLines(db, context.store_id, input) }),
  }),
  tool({
    name: 'get_article_pricing_history',
    title: 'Historique prix article',
    description: 'Retourne l historique journalier d un article: achat, transport, cout rendu et tarifs dynamiques.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.READ,
    requiredPermission: 'pricing.read',
    requiresConfirmation: false,
    inputSchema: pricingReadInputSchema,
    execute: async ({ db, context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Historique article', data: await pricing.getArticlePricingHistory(db, context.store_id, input) }),
  }),
  tool({
    name: 'list_tariff_levels',
    title: 'Lister niveaux tarifaires',
    description: 'Liste les niveaux tarifaires dynamiques, dont les niveaux de compatibilite Tarif 1, 2 et 3.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.READ,
    requiredPermission: 'pricing.read',
    requiresConfirmation: false,
    inputSchema: pricingReadInputSchema,
    execute: async ({ db, context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Niveaux tarifaires', data: await pricing.listTariffLevels(db, context.store_id, input) }),
  }),
  tool({
    name: 'list_supplier_price_imports',
    title: 'Lister imports cours fournisseur',
    description: 'Liste les imports de cours fournisseur et leurs compteurs de rapprochement.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.READ,
    requiredPermission: 'pricing.read',
    requiresConfirmation: false,
    inputSchema: pricingReadInputSchema,
    execute: async ({ db, context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Imports fournisseur', data: await pricing.listSupplierPriceImports(db, context.store_id, input) }),
  }),
  tool({
    name: 'get_supplier_price_import',
    title: 'Lire import cours fournisseur',
    description: 'Lit un import de cours fournisseur avec lignes, statut de matching et article propose.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.READ,
    requiredPermission: 'pricing.read',
    requiresConfirmation: false,
    inputSchema: pricingReadInputSchema,
    execute: async ({ db, context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Import fournisseur', data: await pricing.getSupplierPriceImport(db, context.store_id, input) }),
  }),
  tool({
    name: 'list_supplier_price_import_lines',
    title: 'Lister lignes import fournisseur',
    description: 'Liste les lignes d un import fournisseur avec matching automatique et decision humaine separes.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.READ,
    requiredPermission: 'pricing.read',
    requiresConfirmation: false,
    inputSchema: pricingReadInputSchema,
    execute: async ({ db, context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Lignes import fournisseur', data: await pricing.listSupplierPriceImportLines(db, context.store_id, input) }),
  }),
  tool({
    name: 'search_supplier_import_unresolved_lines',
    title: 'Chercher lignes import a arbitrer',
    description: 'Liste les lignes fournisseur encore en attente de decision humaine.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.READ,
    requiredPermission: 'pricing.read',
    requiresConfirmation: false,
    inputSchema: pricingReadInputSchema,
    execute: async ({ db, context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Lignes import a traiter', data: await pricing.listSupplierPriceImportLines(db, context.store_id, { ...input, unresolved: true }) }),
  }),
  tool({
    name: 'search_articles_for_supplier_mapping',
    title: 'Rechercher article pour mapping fournisseur',
    description: 'Recherche un article ALTA existant par PLU, designation ou famille pour arbitrer une ligne fournisseur.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.READ,
    requiredPermission: 'pricing.read',
    requiresConfirmation: false,
    inputSchema: pricingReadInputSchema,
    execute: async ({ db, context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Articles ALTA', data: await pricing.searchArticlesForSupplierMapping(db, context.store_id, input) }),
  }),
  tool({
    name: 'list_supplier_article_mappings',
    title: 'Lister mappings fournisseur',
    description: 'Liste les correspondances fournisseur/article ALTA normalisees, par fournisseur, designation ou article.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.READ,
    requiredPermission: 'pricing.read',
    requiresConfirmation: false,
    inputSchema: pricingReadInputSchema,
    execute: async ({ db, context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Mappings fournisseur', data: await pricing.searchSupplierArticleMappings(db, context.store_id, input) }),
  }),
  tool({
  ...
  tool({
    name: 'prepare_pricing_session_create',
    title: 'Preparer session tarification',
    description: 'Prepare la creation d un brouillon Tarification pour une date. La fiche appel n est pas modifiee avant publication.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.LOW_REVERSIBLE_WRITE,
    requiredPermission: 'pricing.write',
    requiresConfirmation: false,
    inputSchema: { type: 'object', properties: { date: { type: 'string' }, pricing_date: { type: 'string' }, title: { type: 'string' }, notes: { type: 'string' } }, additionalProperties: false },
    execute: async ({ context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Creation session tarification preparee', data: { prepared_action: pricingPreparedAction(context, input, 'pricing.session.create', 'Creer une session tarification', 'Un brouillon de tarification sera cree pour la date demandee.'), confirmation_tool: 'create_pending_action', action_type: 'pricing.session.create', payload: input } }),
  }),
  tool({
    name: 'prepare_pricing_session_duplicate',
    title: 'Preparer reprise tarification',
    description: 'Prepare une nouvelle session en recopiant la derniere tarification disponible ou une session source precise.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.LOW_REVERSIBLE_WRITE,
    requiredPermission: 'pricing.write',
    requiresConfirmation: false,
    inputSchema: { type: 'object', properties: { date: { type: 'string' }, pricing_date: { type: 'string' }, source_session_id: { type: 'string' }, title: { type: 'string' }, notes: { type: 'string' } }, additionalProperties: false },
    execute: async ({ context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Duplication tarification preparee', data: { prepared_action: pricingPreparedAction(context, input, 'pricing.session.duplicate', 'Reprendre la derniere tarification', 'Une nouvelle session brouillon sera creee sans modifier l historique.'), confirmation_tool: 'create_pending_action', action_type: 'pricing.session.duplicate', payload: input } }),
  }),
  tool({
    name: 'prepare_pricing_session_revision',
    title: 'Preparer revision tarification',
    description: 'Prepare la creation ou reutilisation du brouillon de revision canonique depuis une session tarification publiee.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.LOW_REVERSIBLE_WRITE,
    requiredPermission: 'pricing.write',
    requiresConfirmation: false,
    inputSchema: { type: 'object', required: ['source_session_id'], properties: { source_session_id: { type: 'string' }, pricing_session_id: { type: 'string' }, title: { type: 'string' }, notes: { type: 'string' } }, additionalProperties: false },
    execute: async ({ context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Revision tarification preparee', data: { prepared_action: pricingPreparedAction(context, input, 'pricing.session.revision', 'Creer ou ouvrir une revision tarification', 'Une publication reste historique; le brouillon existant pour cette publication sera reutilise si disponible.', [{ type: 'pricing_session', id: input.source_session_id }]), confirmation_tool: 'create_pending_action', action_type: 'pricing.session.revision', payload: input } }),
  }),
  tool({
    name: 'prepare_pricing_line_add',
    title: 'Preparer ajout ligne tarification',
    description: 'Prepare l ajout d un article ALTA avec fournisseur, prix achat, transport et tarifs dynamiques. Sur session publiee, utiliser create_revision_if_published pour passer par la revision canonique.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.LOW_REVERSIBLE_WRITE,
    requiredPermission: 'pricing.write',
    requiresConfirmation: false,
    inputSchema: pricingLineMutationSchema,
    execute: async ({ context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Ajout ligne tarification prepare', data: { prepared_action: pricingPreparedAction(context, input, 'pricing.line.add', 'Ajouter une ligne tarification', 'Si la session est publiee, la revision brouillon canonique sera creee ou reutilisee avant ajout.', [{ type: 'pricing_session', id: input.pricing_session_id }]), confirmation_tool: 'create_pending_action', action_type: 'pricing.line.add', payload: input } }),
  }),
  tool({
    name: 'prepare_pricing_line_update',
    title: 'Preparer modification ligne tarification',
    description: 'Prepare une modification de ligne: prix achat, transport, fournisseur retenu, exclusion mercuriale et tarifs dynamiques. Sur publication, utiliser create_revision_if_published pour modifier la ligne copiee dans la revision canonique.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.LOW_REVERSIBLE_WRITE,
    requiredPermission: 'pricing.write',
    requiresConfirmation: false,
    inputSchema: { ...pricingLineMutationSchema, required: ['pricing_line_id'] },
    execute: async ({ context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Modification ligne tarification preparee', data: { prepared_action: pricingPreparedAction(context, input, 'pricing.line.update', 'Modifier une ligne tarification', 'Si la ligne appartient a une publication, la revision brouillon canonique sera creee ou reutilisee avant modification.', [{ type: 'pricing_line', id: input.pricing_line_id }]), confirmation_tool: 'create_pending_action', action_type: 'pricing.line.update', payload: input } }),
  }),
  tool({
    name: 'prepare_pricing_line_remove',
    title: 'Preparer retrait ligne tarification',
    description: 'Prepare le retrait d une ligne dans une session brouillon. Une session publiee reste historique et non modifiable.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.LOW_REVERSIBLE_WRITE,
    requiredPermission: 'pricing.write',
    requiresConfirmation: false,
    inputSchema: { type: 'object', required: ['pricing_line_id'], properties: { pricing_line_id: { type: 'string' } }, additionalProperties: false },
    execute: async ({ context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Retrait ligne tarification prepare', data: { prepared_action: pricingPreparedAction(context, input, 'pricing.line.remove', 'Retirer une ligne tarification', 'La ligne sera retiree du brouillon apres confirmation.', [{ type: 'pricing_line', id: input.pricing_line_id }]), confirmation_tool: 'create_pending_action', action_type: 'pricing.line.remove', payload: input } }),
  }),
  tool({
    name: 'prepare_supplier_price_import',
    title: 'Preparer import cours fournisseur',
    description: 'Prepare un import de cours fournisseur depuis texte ou lignes structurees. Matching deterministe uniquement, sans IA externe.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.LOW_REVERSIBLE_WRITE,
    requiredPermission: 'pricing.write',
    requiresConfirmation: false,
    inputSchema: { type: 'object', required: ['supplier_id'], properties: { supplier_id: { type: 'string' }, import_date: { type: 'string' }, source_type: { type: 'string' }, raw_text: { type: 'string' }, lines: { type: 'array', items: { type: 'object', additionalProperties: true } } }, additionalProperties: false },
    execute: async ({ context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Import fournisseur prepare', data: { prepared_action: pricingPreparedAction(context, input, 'pricing.supplier_import.create', 'Importer un cours fournisseur', 'Les lignes seront parsees et rapprochees sans creer automatiquement d article.'), confirmation_tool: 'create_pending_action', action_type: 'pricing.supplier_import.create', payload: input } }),
  }),
  tool({
    name: 'prepare_supplier_import_apply',
    title: 'Preparer application import',
    description: 'Prepare l application des lignes validees humainement d un import fournisseur vers une session tarification brouillon. Si la session cible est publiee, create_revision_if_published reutilise ou cree la revision brouillon canonique avant application.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.LOW_REVERSIBLE_WRITE,
    requiredPermission: 'pricing.write',
    requiresConfirmation: false,
    ...
    execute: async ({ context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Application import preparee', data: { prepared_action: pricingPreparedAction(context, input, 'pricing.supplier_import.apply', 'Appliquer un import fournisseur', 'Si la session est publiee, la revision brouillon canonique sera creee ou reutilisee uniquement quand create_revision_if_published vaut true. Seules les lignes confirmees ou corrigees humainement alimenteront le brouillon de tarification.'), confirmation_tool: 'create_pending_action', action_type: 'pricing.supplier_import.apply', payload: input } }),
  }),
  tool({
    name: 'prepare_supplier_import_line_confirm',
    title: 'Preparer confirmation ligne import',
    description: 'Prepare la confirmation humaine de l article propose pour une ligne import fournisseur et memorise le mapping fournisseur.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.LOW_REVERSIBLE_WRITE,
    requiredPermission: 'pricing.write',
    requiresConfirmation: false,
    inputSchema: { type: 'object', required: ['import_line_id'], properties: { import_line_id: { type: 'string' }, article_id: { type: 'string' } }, additionalProperties: false },
    execute: async ({ context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Confirmation ligne preparee', data: { prepared_action: pricingPreparedAction(context, input, 'pricing.supplier_import_line.confirm', 'Confirmer une ligne import fournisseur', 'Le mapping sera memorise pour ce fournisseur apres confirmation humaine.', [{ type: 'supplier_price_import_line', id: input.import_line_id }]), confirmation_tool: 'create_pending_action', action_type: 'pricing.supplier_import_line.confirm', payload: input } }),
  }),
  tool({
    name: 'prepare_supplier_import_line_override',
    title: 'Preparer correction ligne import',
    description: 'Prepare le remplacement humain de l article associe a une ligne import fournisseur.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.LOW_REVERSIBLE_WRITE,
    requiredPermission: 'pricing.write',
    requiresConfirmation: false,
    inputSchema: { type: 'object', required: ['import_line_id', 'article_id'], properties: { import_line_id: { type: 'string' }, article_id: { type: 'string' } }, additionalProperties: false },
    execute: async ({ context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Correction ligne preparee', data: { prepared_action: pricingPreparedAction(context, input, 'pricing.supplier_import_line.override', 'Changer le mapping d une ligne import', 'Le nouveau mapping remplacera le mapping actif pour ce fournisseur et cette designation.', [{ type: 'supplier_price_import_line', id: input.import_line_id }, { type: 'article', id: input.article_id }]), confirmation_tool: 'create_pending_action', action_type: 'pricing.supplier_import_line.override', payload: input } }),
  }),
  tool({
    name: 'prepare_supplier_import_line_ignore',
    title: 'Preparer ignore ligne import',
    description: 'Prepare l exclusion humaine d une ligne import fournisseur de l application a la session.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.LOW_REVERSIBLE_WRITE,
    requiredPermission: 'pricing.write',
    requiresConfirmation: false,
    inputSchema: { type: 'object', required: ['import_line_id'], properties: { import_line_id: { type: 'string' } }, additionalProperties: false },
    execute: async ({ context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Ignore ligne preparee', data: { prepared_action: pricingPreparedAction(context, input, 'pricing.supplier_import_line.ignore', 'Ignorer une ligne import fournisseur', 'La ligne ne sera pas appliquee a la session tarification.', [{ type: 'supplier_price_import_line', id: input.import_line_id }]), confirmation_tool: 'create_pending_action', action_type: 'pricing.supplier_import_line.ignore', payload: input } }),
  }),
  tool({
    name: 'prepare_supplier_article_mapping_confirm',
    title: 'Preparer confirmation mapping',
    description: 'Prepare la creation ou correction d une correspondance fournisseur vers article ALTA. Ne cree jamais de nouvel article.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.LOW_REVERSIBLE_WRITE,
    requiredPermission: 'pricing.write',
    requiresConfirmation: false,
    inputSchema: { type: 'object', required: ['supplier_id', 'article_id', 'supplier_designation_original'], properties: { supplier_id: { type: 'string' }, article_id: { type: 'string' }, supplier_designation_original: { type: 'string' }, supplier_ref: { type: 'string' }, supplier_label: { type: 'string' }, mapping_source: { type: 'string' }, confidence_score: { type: 'number' } }, additionalProperties: false },
    execute: async ({ context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Mapping fournisseur prepare', data: { prepared_action: pricingPreparedAction(context, input, 'pricing.supplier_mapping.upsert', 'Confirmer un mapping fournisseur', 'La correspondance sera memorisee pour ce fournisseur uniquement.'), confirmation_tool: 'create_pending_action', action_type: 'pricing.supplier_mapping.upsert', payload: input } }),
  }),
  tool({
    name: 'prepare_pricing_session_publish',
    title: 'Preparer publication tarification',
    description: 'Prepare la publication transactionnelle d une session. Elle devient la reference active par magasin/date et synchronise le miroir fiche appel.',
    domain: 'pricing',
    riskLevel: RISK_LEVELS.LOW_REVERSIBLE_WRITE,
    requiredPermission: 'pricing.write',
    requiresConfirmation: false,
    inputSchema: { type: 'object', required: ['pricing_session_id'], properties: { pricing_session_id: { type: 'string' }, sync_call_sheet: { type: 'boolean' } }, additionalProperties: false },
    execute: async ({ context, input, tool: currentTool }) => response({ tool: currentTool.name, domain: currentTool.domain, summary: 'Publication tarification preparee', data: { prepared_action: pricingPreparedAction(context, input, 'pricing.session.publish', 'Publier les tarifs du jour', 'La publication est transactionnelle et remplacera la publication active de la meme date.', [{ type: 'pricing_session', id: input.pricing_session_id }]), confirmation_tool: 'create_pending_action', action_type: 'pricing.session.publish', payload: input } }),
  }),
  tool({
