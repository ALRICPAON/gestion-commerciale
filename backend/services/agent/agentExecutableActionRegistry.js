...
const ACTIONS = [
  ...
  {
    name: 'pricing.session.revision',
    description: 'Cree ou reutilise le brouillon de revision canonique depuis une session publiee. La publication source reste immuable.',
    aliases: [],
    module: 'pricing',
    requiredPermission: 'pricing.write',
    requiredPermissions: ['mcp.execute', 'pricing.write'],
    service: 'pricingService.getOrCreateDraftRevisionFromPublishedSession',
    confirmationLevel: 'explicit_human',
    reversible: true,
    previewRequired: true,
    batch: false,
    payloadSchema: { type: 'object', required: ['source_session_id'], properties: { source_session_id: { type: 'string' }, pricing_session_id: { type: 'string' }, title: { type: 'string' }, notes: { type: 'string' } }, additionalProperties: false },
    example: { action_type: 'pricing.session.revision', payload: { source_session_id: 'uuid-session-publiee' } },
    validatePayload: (payload) => { assertObject(payload, 'payload'); return payload; },
    execute: async ({ db, context, payload }) => ({ ok: true, mode: 'executed', action: 'pricing.session.revision', module: 'pricing', result: await pricing.getOrCreateDraftRevisionFromPublishedSession(db, context.store_id, payload, context) }),
  },
  {
    name: 'pricing.line.add',
    description: 'Ajoute une ligne dans une session brouillon avec cout rendu calcule cote service et tarifs dynamiques. Si la session cible est publiee, create_revision_if_published reutilise ou cree le brouillon canonique avant ajout.',
    aliases: [],
    module: 'pricing',
    requiredPermission: 'pricing.write',
    requiredPermissions: ['mcp.execute', 'pricing.write'],
    ...
    example: { action_type: 'pricing.line.add', payload: { pricing_session_id: 'uuid-session', article_id: 'uuid-article', purchase_price_ht: 5.5, create_revision_if_published: true, tariffs: [{ legacy_level: 1, price_ht: 7.9 }] } },
    validatePayload: (payload) => { assertObject(payload, 'payload'); return payload; },
  ...
  },
  {
    ...
    description: 'Modifie une ligne de session brouillon. Si la ligne appartient a une publication, create_revision_if_published reutilise ou cree le brouillon canonique puis modifie la ligne copiee. Les champs absents ne changent pas; les tarifs sont stockes dans pricing_line_tariffs.',
    aliases: [],
    module: 'pricing',
    requiredPermission: 'pricing.write',
    requiredPermissions: ['mcp.execute', 'pricing.write'],
    ...
    example: { action_type: 'pricing.line.update', payload: { pricing_line_id: 'uuid-ligne', purchase_price_ht: 5.5, transport_cost_ht: 0.2, create_revision_if_published: true, tariffs: [{ legacy_level: 1, price_ht: 7.9 }] } },
    validatePayload: (payload) => { assertObject(payload, 'payload'); return payload; },
  ...
  },
  {
    ...
    description: 'Applique les lignes confirmees ou corrigees humainement d un import fournisseur a une session brouillon, ou reutilise/cree la revision brouillon canonique si la session cible est publiee et que create_revision_if_published vaut true.',
    aliases: [],
    module: 'pricing',
    requiredPermission: 'pricing.write',
    requiredPermissions: ['mcp.execute', 'pricing.write'],
    ...
    previewRequired: true,
    batch: false,
    payloadSchema: { type: 'object', required: ['import_id', 'pricing_session_id'], properties: { import_id: { type: 'string' }, pricing_session_id: { type: 'string' }, create_revision_if_published: { type: 'boolean' } }, additionalProperties: false },
    example: { action_type: 'pricing.supplier_import.apply', payload: { import_id: 'uuid-import', pricing_session_id: 'uuid-session', create_revision_if_published: true } },
    validatePayload: (payload) => { assertObject(payload, 'payload'); return payload; },
